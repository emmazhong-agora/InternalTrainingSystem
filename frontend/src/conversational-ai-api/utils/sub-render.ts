...
// Simple logger replacement for @agora-js/report
const agoraLogger = {
  info: (...args: any[]) => {}, // No-op, we already log via console
  warn: (...args: any[]) => {},
  error: (...args: any[]) => {},
  debug: (...args: any[]) => {},
};
...
const logger = {
  info: (...args: any[]) => console.log('[SubRender]', ...args),
  warn: (...args: any[]) => console.warn('[SubRender]', ...args),
  error: (...args: any[]) => console.error('[SubRender]', ...args),
  debug: (...args: any[]) => console.debug('[SubRender]', ...args),
}
...
export class CovSubRenderController {
  ...
  protected handleMessageInterrupt(uid: string, message: IMessageInterrupt) {
    ...
    const turn_id = message.turn_id
    // workaround: pts < interrupt.start_ms, and interrupt will be ignored
    const start_ms = min([message.start_ms, this._pts]) || message.start_ms
  ...
  public handleAgentStatus(metadata: IPresenceState) {
    ...
    const currentTurnId = toNumber(message.turn_id) || 0
    if (toNumber(this._agentMessageState?.turn_id || 0) > currentTurnId) {
      this.callMessagePrint(
        ELoggerType.debug,
        'handleAgentStatus',
    ...
    if (toNumber(this._agentMessageState?.timestamp || 0) >= currentMsgTs) {
      // console.debug(
      //   CONSOLE_LOG_PREFIX,
      //   'handleAgentStatus',
    ...
    this.onAgentStateChanged?.(metadata.publisher, {
      state: message.state,
      turnID: toNumber(message.turn_id),
      timestamp: currentMsgTs,
      reason: ''
    })
  }
