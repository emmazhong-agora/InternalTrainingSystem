...
class RtmTokenBuilder:
    ...
    def build_token(app_id, app_certificate, user_id, expire):
        ...
        :param user_id: The user's account (string or integer). If integer, will be converted to string.
        ...
        # Convert user_id to string if it's an integer (for UID consistency with RTC)
        user_id_str = str(user_id) if isinstance(user_id, int) else user_id

        rtm_service = ServiceRtm(user_id_str)
